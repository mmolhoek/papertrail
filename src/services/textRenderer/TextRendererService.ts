 ...
 * Uses SVG generation and sharp for bitmap conversion.
 */
export class TextRendererService implements ITextRendererService {
  ...
  /**
   * Convert SVG string to 1-bit bitmap using Sharp or ImageMagick
   */
  private async svgToBitmap(
    ...
    if (USE_IMAGEMAGICK) {
      return this.svgToBitmapMagick(svgString, width, height);
    }
    return this.svgToBitmapSharp(svgString, width, height);
  }

  /**
   * Convert SVG string to 1-bit bitmap using Sharp
   */
  private async svgToBitmapSharp(
    svgString: string,
    width: number,
    height: number,
  ): Promise<Bitmap1Bit> {
    logger.debug("Converting SVG to 1-bit bitmap using Sharp");
  ...
  /**
   * Convert SVG string to 1-bit bitmap using ImageMagick (wasm-imagemagick)
   */
  private async svgToBitmapMagick(
    svgString: string,
    width: number,
    height: number,
  ): Promise<Bitmap1Bit> {
    ...
    const svgContent = new TextEncoder().encode(svgString);
    const inputFiles = [
      {
        name: "input.svg",
        content: svgContent,
    ...
    if (result.exitCode !== 0) {
      throw new Error(
        `ImageMagick convert failed: ${result.stderr.join("\n")}`,
      );
    }
    ...
    return {
      ...
      metadata: {
        createdAt: new Date(),
        description: "Rendered text template (ImageMagick)",
      },
    };
  }
}
